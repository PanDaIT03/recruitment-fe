...
import { CompanyLogo, Lightning } from '~/assets/svg';
    ...
    <>
      ...
      {appliedJobs?.items?.length ? (
        <List
          ...
          pagination={false}
          loading={isPending}
          dataSource={appliedJobs?.items}
          customSkeleton={
            <Skeleton avatar active title={false} paragraph={{ rows: 2 }} />
          }
          renderItem={(item) => (
            <List.Item>
              <Flex gap={12} className="max-sm:flex-col">
                {item?.job?.user?.avatarUrl ? (
                  <Avatar
                    shape="square"
                    className="w-16 h-16 min-w-16"
                    src={item?.job?.user?.avatarUrl}
                  />
                ) : (
                  <CompanyLogo />
                )}
